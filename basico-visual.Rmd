...
As figuras abaixo mostram imagens relativamente similares ao que os usuários do R padrão (Figura \@ref(fig:rpadrao)) e RStudio (Figura \@ref(fig:rstudio)) verão.  

```{r rpadrao, fig.cap = ' Imagem de uma sessão do R para Mac OSX com um janela de script aberta além do console.', eval = TRUE, echo = FALSE}
knitr::include_graphics("figuras/rpadrao.png")
```

```{r rstudio, fig.cap = ' Imagem de uma sessão do R studio com três painéis abertos.', eval = TRUE, echo = FALSE}
...
Note também que dependendo do objeto, o código acima tem cores diferentes.
Em vermelho e entre aspas estão textos, os números em roxo e objetos criados em preto.
...
O conteúdo do script será executado no Console. Note que o símbolo `#` não é interpretado pelo console.  
...
+ Funções do pacote Base - funções que vêm junto com o R quando você faz o download do programa;
...
Por exemplo, para trabalhar com dados filogenéticos, você pode necessitar do pacote [ape](https://cran.r-project.org/web/packages/ape/index.html) (Analyses of Phylogenetic and Evolution), que já tem várias funções preparadas para análises filogenéticas.  
...
Ao digitar uma das opções acima, o R vai abrir uma janela (no RStudio, vai colocar esse resultado num dos painéis) contendo a página com a explicação da função (Veja abaixo).
Todos esses trechos de ajuda (`?`) do R têm a mesma estrutura.  
...
A grande vantagem de realizar a análise de dados, produção de figuras, etc no R (e RStudio) é criar um processo replicável, e é preciso ser bem organizado com os arquivos para poder reutilizar o processo com novos dados ou adaptá-lo para novas análises. Isso implica, entre outras coisas, em:
...
O RStudio facilita esse processo para você, permitindo que você crie um projeto.
...
```{r, opts.label='evalF', include=FALSE}
