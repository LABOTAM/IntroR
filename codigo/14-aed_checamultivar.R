...
#' Há também a função `vegdist()` do pacote `vegan` que faz a mesma coisa, mas tem índices de dissimilaridade que `dist()` não implementa, muito dos quais muito usados em ecologia de comunidades.  
#' 
...
#' 
#' Vamos utilizar o conjunto de [dados contendo coordenadas geográficas de municípios do Brasil](https://github.com/LABOTAM/LABOTAM.github.io/blob/main/dados/municipiosbrasil.csv) para esta prática, utilizado na seção \@ref(import-dados).  
#' 
## ---- eval = FALSE------------------------------------------------------------
...
#' 
## -----------------------------------------------------------------------------
# para simplificar vamos pegar apenas algumas cidades da região norte
...
#' 
#' Um exemplo de NMDS para mostra a similaridade entre indivíduos por variáveis morfológicas usando os dados de `iris` do R.  
#' 
## -----------------------------------------------------------------------------
# apaga todos os objetos
rm(list = ls())
# agora vamos usar os dados de iris
head(iris)
# vamos apresentar a similaridade entre as instancias de iris
# a partir de uma matriz de distancia morfológica
...
plot(ptsnmds, pch = 21, bg = cores, cex = 0.8, xlab = "NMDS 1", ylab = "NMDS 2")
...
#' Vamos carregar o pacote `vegan` [@R-vegan]
#' 
## -----------------------------------------------------------------------------
library("vegan")
...
plot(ptsnmds2, pch = 21, bg = cores, cex = 0.8, xlab = "NMDS 1", ylab = "NMDS 2")
...
# usando o método de agrupamento pelos centroides
gp3 <- hclust(mdist, method = "centroid")
plot(gp3, hang = 0.1, main = "Cidades da região norte", ylab = "Distância geográfica (dd)")

# teste outros métodos (entenda eles)

#' 
#' 
## -----------------------------------------------------------------------------
# AGORA TRABALHANDO O RESULTADO DE hclust COMO UM OBJETO DE CLASSE PHYLO (de filogenia) do pacote ape
library("ape")
...
#' 
## -----------------------------------------------------------------------------
library("vegan") # precisa desse pacote
...
#' 
## -----------------------------------------------------------------------------
# podemos fazer um NMDS com isso
library("labdsv")
...
## ---- eval = FALSE------------------------------------------------------------
## # melhorando a figura
## # primeiro converto para phylo
## # é uma forma de fazer isso
## # veja também ?dendrogram
## library("ape")
...
## # ERRO AQUi - INVESTIGAR
## #########################
## # agora fazendo a mesma coisa considerando a abundancia (usamos o indice de sorensen)
## # nosso objeto com abundancia por parcela é tb
## tb[1:4, 1:5]
## 
