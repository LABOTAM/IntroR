...
Vejamos abaixo quando utilizamos esses valores em letras minúsculas:

```{r, echo = TRUE, eval = TRUE}
v5 <- c(true, true, false, false) 
```

O R não entende isso como valores lógico, pois ele procura no ambiente por objetos com esses nomes.
Por isso ele retorna a mensagem de erro `Error: object 'true' not found` pois ele procura pelo primeiro objeto de nosso vetor ` c(true, true, false, false)` na área de trabalho e, ao não encontrar, ele retorna esta mensagem de erro.
