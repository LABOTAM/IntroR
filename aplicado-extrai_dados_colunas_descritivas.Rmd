...
Os scripts aqui relacionados mostram exemplos de como extrair informações dessas colunas descritivas, categorizando informações de __habito__, __fertilidade__, __textura do solo__, __habitat__, __dap__, __altura__.

## Dados categóricos

A lógica básica é simples. 
Para cada tipo de informação (variável) você busca por um conjunto de palavras e suas variantes, e substitui todas elas por uma única palavra, criando estados de variação (categorias).
...
A construção dessa lista deve ser baseada nas palavras que existem nos seus dados. As listas de referência abaixo contêm alguns conjuntos de palavras que você pode ajustar para seu uso.  

```{r listas, opts.label='evalF'}
#exemplo com a variável habito com duas categorias que desejamos separar: arvoreta e arvore
#criamos a lista de referencia obj[[nome.da.categoria]] = c(palavras.a.buscar):
referencias = list()
...
#primeiro arvoreta e depois arvore, porque __arvore__ faz parte da palavra arvoreta e quero diferenciar as duas. Note que erros de grafia podem ser inseridos nesses vetores como palavras a buscar e diferentes idiomas podem ser buscados
```

### Passo 02 - Funções que fazem a busca

Primeiro criamos uma função que pega um texto ou vetor de textos e remove acentos.
...
E uma segunda função que usa a lista de referência para extrair os dados de um texto.  

```{r, opts.label='evalF'}
#x é um vetor character de comprimento=1 com o texto original a ser explorado
#referencias é uma lista de comprimento >=1 referente as categorias a serem buscadas em x
...
dados = read.table(file="pegados_dados_exemplo.csv",sep='\t',as.is=T,na.strings=c("NA","NULL",""),header=T)
...
Funções para extrair altura e DAP de colunas descritivas.  
  ...
  altura = NA
    for(p in 1:57) {
        ...
        if (length(gp)>0 & is.na(altura)) {
            ...
            xx = mytrim(gsub("[A-Z]|\\(|\\)|:|=","",xx,ignore.case = T))
            xx = gsub(",",".",xx)
            tt = grep("-",xx)
            if (length(tt)>0) { 
                xxx = strsplit(xx,"-")[[1]]
                xxx = xxx[xxx!=""]
                xxx = gsub(" \\.","",xxx)
                xx = mean(as.numeric(mytrim(xxx)),na.rm=T)
                } else {
                    xx = strsplit(xx," ")[[1]]
                    xx = mytrim(xx)
                    xx = xx[xx!="."]
...
```{r, opts.label='evalF'}
# usando os dados de exemplo carregados acima
